    ...
    def mise_a_jour(self,values):
        ...
        for key, element in values.items():
            if f := self.dico_fonction.get(key):
                f(element)
            else : my_logger.warning(f"element de donnée incompatible : cle = {key} valeur = {element} in PID {os.getpid()}")
        ...
        try:
            ...
            my_logger.warning(f"Echec de l'envoi de données = {self.num_tag_str} in PID {os.getpid()}")
