...
def log_message_debug(message):
    ...
    logger.debug(f"f{message} in PID {os.getpid()}")
...
def log_message_info(message):
    logger = logging.getLogger("app")
    logger.info(f"f{message} in PID {os.getpid()}")
#    logging.info(message)
    print(message) 

def log_message_warning(message):
    logger = logging.getLogger("app")
    logger.warning(f"f{message} in PID {os.getpid()}")
#    logging.warning(message)
    print(message) 

def log_message_error(message):
    logger = logging.getLogger("app")
    logger.error(f"f{message} in PID {os.getpid()}")
#    logging.error(message)
    print(message) 

def log_message_critical(message):
    logger = logging.getLogger("app")
    logger.critical(f"f{message} in PID {os.getpid()}")
...
#    logging.basicConfig(filename=logfilename,level=logging.DEBUG,format=format_logging)
    ...
    log_file = f'{real_path}/Logs/Serveur.log'
